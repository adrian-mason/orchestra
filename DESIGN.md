...
orchestra_workflow = Workflow(
    ...
    steps=[
        ...
        # Stage 2: Design — 产出写入 session_state["latest_design_content"]
        ...
        Loop(
            ...
            steps=[
                # 1. 审查当前设计（读 session_state["latest_design_content"]）
                Step(executor=design_review_team, name="design_review"),
                # 2. 检查审查结果 — 全部 APPROVED 则退出循环
                ...
                #    修订后更新 session_state["latest_design_content"]
        ...
        # 通过 → 继续；失败 → 函数内部 raise WorkflowPauseRequest 暂停 workflow
        Step(executor=final_integration_review, name="integration_review"),

        # Stage 5: PR Lifecycle
        # 只有 integration_review 正常返回（通过）时才会执行到这里。
        # 失败时 workflow 已被 WorkflowPauseRequest 暂停，不会到达此步。

        # 5a: PR 创建 + CI 监控（PR Shepherd 自治状态机）
        Step(executor=pr_shepherd, name="pr_lifecycle"),

        # 5b: 持久化 PR 上下文（PR URL → session_state，供 merge 使用）
        ...
        Step(executor=check_pr_review_gate, name="critic_gate"),    # 阻塞性 gate
        Step(executor=challenger, name="challenger_review"),
        Step(executor=check_pr_review_gate, name="challenger_gate"),  # 阻塞性 gate

        # 5d: Merge（仅在 critic + challenger 都 APPROVED 后）
...
```python
class WorkflowPauseRequest(Exception):
    """Raise in any step function to suspend the workflow run.

    Workflow engine 捕获此异常后：
    1. 将 run 状态持久化到 checkpoint（当前 step name + session_state）
    2. 将 run 标记为 status="paused"
    3. 停止执行后续 steps（不是 noop，是真正停止）

    人工通过 /decision-gates/{id}/resolve 审批后，
    workflow_engine.resume(run_id) 从 checkpoint 恢复，
    重新执行当前 step（此时 gate 已 resolved，step 正常返回）。
    """
    def __init__(self, decision_gate_id: str, reason: str):
        self.decision_gate_id = decision_gate_id
        self.reason = reason

def final_integration_review(step_input: StepInput) -> StepOutput:
    """跨 work unit 集成审查 — 阻塞性 gate。

    通过：正常返回 StepOutput，workflow 继续到 PR Lifecycle。
    失败：创建 DecisionGate + raise WorkflowPauseRequest，workflow 真正暂停。
    恢复：人工 resolve gate 后，workflow_engine.resume() 重新执行此 step，
         此时检测到 gate 已 resolved，直接返回通过。
    """
    run_id = step_input.metadata["run_id"]

    # 先检查是否是 resume 后重入——如果已有 resolved gate，直接放行
    existing_gate = events_db.query_one(
        "SELECT * FROM decision_gates WHERE workflow_run_id = ? "
        "AND gate_type = 'integration' AND status IN ('approved', 'override')",
        (run_id,),
    )
    if existing_gate:
        return StepOutput(
            content="INTEGRATION_APPROVED (via Decision Gate override)",
    ...
    if verdict.verdict == "APPROVED" and integration_passed:
        return StepOutput(
            content="INTEGRATION_APPROVED",
            metadata={"integration_passed": True},
        )

    # 集成失败 → 通过统一 helper 创建 Decision Gate 并暂停 workflow
    ...
    raise WorkflowPauseRequest(
        decision_gate_id=gate.id,
        reason=f"Integration review failed: {len(verdict.blockers)} blockers",
    )
```

**暂停/恢复机制**：`WorkflowPauseRequest` 是 step 函数暂停 workflow 的唯一正确方式。Workflow engine 捕获后持久化 checkpoint 并停止。`resume()` 从 checkpoint 重新执行当前 step——step 内部检测到 gate 已 resolved 则正常返回，workflow 继续。所有 Decision Gate 场景（plan_review / design_review / integration / pr_review / pr_merge）统一使用此机制。
...
def check_pr_review_gate(step_input: StepInput) -> StepOutput:
    """程序化消费 reviewer 的 GateVerdict — 阻塞性 gate。

    此函数跟在每个 reviewer 后面（critic_gate / challenger_gate），
    解析 previous_step_content 中的 GateVerdict JSON：
    - APPROVED → 通过，继续下一步
    - NEEDS_REVISION → raise WorkflowPauseRequest，人工决定修复或跳过

    gate_type 统一为 "pr_review"（与 DecisionGate.gate_type Literal 一致），
    reviewer 身份通过 context.reviewer_step 区分。
    """
    run_id = step_input.metadata["run_id"]
    reviewer_step = step_input.metadata.get("step_name", "pr_review")

    # resume 重入检测 — 按 run_id + gate_type + context.reviewer_step 匹配
    existing_gate = events_db.query_one(
        "SELECT * FROM decision_gates WHERE workflow_run_id = ? "
        "AND gate_type = 'pr_review' "
        "AND json_extract(context, '$.reviewer_step') = ? "
        "AND status IN ('approved', 'override')",
        (run_id, reviewer_step),
    )
    if existing_gate:
        return StepOutput(content="GATE_PASSED (via override)", metadata={"gate_passed": True})
    ...
    # reviewer 不通过 → 创建 Decision Gate 暂停
    # gate_type = "pr_review"（合法 Literal），reviewer_step 通过 extra_context 一次性写入
    gate = create_decision_gate(
    ...
    raise WorkflowPauseRequest(
        decision_gate_id=gate.id,
        reason=f"PR review ({reviewer_step}) failed: {verdict.blockers}",
    )
...
def execute_merge(step_input: StepInput) -> StepOutput:
    ...
    遵循统一的 Decision Gate resume 协议（同 plan/design/integration gate）：
    首次执行 → 创建 gate + pause；resume 重入 → 检测 gate 已 approved → 执行合并。
    """
    run_id = step_input.metadata["run_id"]

    # Resume 重入检测（与 check_plan_gate / check_design_gate / integration_review 一致）
    existing_gate = events_db.query_one(
        "SELECT * FROM decision_gates WHERE workflow_run_id = ? "
        "AND gate_type = 'pr_merge' AND status IN ('approved', 'override')",
        (run_id,),
    )
    # 防御性校验 — checkpoint 恢复或手动调用可能丢失 session_state
    pr_url = step_input.session_state.get("pr_url", "")
    if not pr_url or not pr_url.startswith("https://"):
        raise ValueError(f"execute_merge: missing or invalid pr_url in session_state: '{pr_url}'")
    ...
    if config.get("auto_merge", False):
        # 自动合并 — 高信任项目，跳过 Decision Gate
        github_merge(pr_url)
        emit_event("merge_attempt", pr_url=pr_url, result="success")
        return StepOutput(content=f"Merged: {pr_url}")

    # 默认：HITL 最终确认（复用上面已校验的 pr_url）
    gate = create_decision_gate(
        step_input, gate_type="pr_merge", verdicts=[],
        extra_context={"pr_url": pr_url},
    )
    raise WorkflowPauseRequest(
        decision_gate_id=gate.id,
        reason=f"PR {pr_url} ready to merge — awaiting human confirmation",
    )
```
...
def check_plan_gate(step_input: StepInput) -> StepOutput:
    ...
    # resume 后重入检测（同 final_integration_review 模式）
    run_id = step_input.metadata["run_id"]
    existing_gate = events_db.query_one(
        "SELECT * FROM decision_gates WHERE workflow_run_id = ? "
        "AND gate_type = 'plan_review' AND status IN ('approved', 'override')",
        (run_id,),
    )
    if existing_gate:
        return StepOutput(content="GATE_PASSED (via override)", metadata={"gate_passed": True})
    ...
    round_num = step_input.metadata.get("round", 0) + 1

    if all_pass:
        # Plan gate 通过 — 不写 approved_design，那是 design gate 的职责
        return StepOutput(content="GATE_PASSED", metadata={"gate_passed": True})

    if round_num >= 3:
        # 3 轮后 — 创建 Decision Gate 并真正暂停 workflow
        gate = create_decision_gate(
            step_input, gate_type="plan_review", verdicts=verdicts
        )
        raise WorkflowPauseRequest(
            decision_gate_id=gate.id,
            reason=f"Plan review failed after {round_num} rounds",
        )
...
def check_design_gate(step_input: StepInput) -> StepOutput:
    ...
    这是 approved_design 写入 session_state 的唯一位置。
    ...
    existing_gate = events_db.query_one(
        "SELECT * FROM decision_gates WHERE workflow_run_id = ? "
        "AND gate_type = 'design_review' AND status IN ('approved', 'override')",
        (run_id,),
    )
    if existing_gate:
        # 人工 override 后仍需写入 approved_design
        step_input.session_state["approved_design"] = (
            step_input.session_state.get("latest_design_content", "")
        )
        return StepOutput(content="GATE_PASSED (via override)", metadata={"gate_passed": True})
    ...
    if all_approved:
        ...
        # latest_design_content 由 persist_design_output（首次）和 revise_design_from_feedback（每轮修订）写入
        step_input.session_state["approved_design"] = (
            step_input.session_state.get("latest_design_content", "")
        )
        return StepOutput(content="GATE_PASSED", metadata={"gate_passed": True})
    ...
    if round_num >= 3:
        gate = create_decision_gate(
            step_input, gate_type="design_review", verdicts=verdicts
        )
        raise WorkflowPauseRequest(
            decision_gate_id=gate.id,
            reason=f"Design review failed after {round_num} rounds",
        )
...
1. Design Team 产出 `final_design.md` → 写入 `session_state["latest_design_content"]`
2. Plan Review Gate 验证计划可行性（不写 `approved_design`）
3. Design Review Gate 验证设计完整性 → **通过时写入 `session_state["approved_design"]`**
4. `github_setup` 和 `decompose_work_units` 从 `session_state["approved_design"]` 读取
...
def create_decision_gate(
    ...
    调用者：check_plan_gate / check_design_gate / final_integration_review /
           check_pr_review_gate / execute_merge。
    创建后由调用方 raise WorkflowPauseRequest 暂停 workflow。
...
Decision Gate 是一个**可持久化、可观测的协议对象**，而非简单的元数据标记。它连接 Review Gate 的人工升级、Watchdog 的 Decision Gate 感知、以及 REST API 人工审批的暂停/恢复（`WorkflowPauseRequest` + `resolve_gate`）。
...
Gate step 在需要人工升级或确认时创建 `DecisionGate` 并 `raise WorkflowPauseRequest`。触发条件因场景而异：plan/design review 是 3 轮失败后升级，pr_review 是 reviewer verdict 不通过立即升级，pr_merge 是默认要求人工最终确认。所有场景遵循同一暂停/恢复模式：

1. **首次执行**：step 逻辑判定需要人工介入 → 创建 gate → `raise WorkflowPauseRequest`
2. **Workflow engine**：捕获异常 → 持久化 checkpoint → 标记 run `status="paused"`
3. **人工审批**：通过 REST API resolve gate
4. **Resume**：`workflow_engine.resume(run_id)` → 从 checkpoint 重新执行当前 step
5. **重入执行**：step 检测到 gate 已 `approved` → 直接返回通过
...
| 阶段 | 协作模式 | 原因 |
|------|----------|------|
| Design | `coordinate` | Maestro 分解+综合，成员串行/条件执行 |
| Plan Review | `broadcast` | 所有审查者同时收到同一文档 |
| Design Review | `broadcast` | 所有专家并行审查 |
| Implementation | `route` | 按 task 类型路由到最合适的 specialist |
| PR Review | 独立 Agent 串行 | HITL 在 Team 层尚未完全支持，暂用 Workflow Step 串行调用独立 Agent（见 10.4） |
...
来源：Agno F6 修正 + Stage 5 PR Lifecycle 设计

Orchestra 的 HITL 采用 **Decision Gate 机制**（见 4.6），而非 Agno 原生 `@tool(requires_confirmation=True)`。原因：

1. Agno HITL 在 Team/Workflow 层面标注 "near future"，当前仅 Agent 层稳定
2. Orchestra 需要跨步骤的暂停/恢复语义（`WorkflowPauseRequest` + checkpoint）
3. Decision Gate 提供统一的 REST API 审批、Watchdog 感知、可观测性

**PR Merge 流程**（详见 2.7 Stage 5）：
- Critic/Challenger 各返回 GateVerdict → `check_pr_review_gate` 程序化消费
- 审查不通过 → `DecisionGate(gate_type="pr_review")` + `WorkflowPauseRequest`
- 最终合并 → `execute_merge()` 默认创建 `DecisionGate(gate_type="pr_merge")` 等待人工确认
- 高信任项目可配 `auto_merge: true` 跳过最终确认

⚠ **未来迁移**：当 Agno HITL 在 Workflow 层稳定后，可将 `check_pr_review_gate` 和 `execute_merge` 中的 Decision Gate 替换为 Agno 原生 `@tool(requires_confirmation=True)` + `@approval`。Decision Gate 协议保留用于非 HITL 场景（如 integration review 失败的人工升级）。
...
| 风险 | 影响 | 概率 | 缓解 |
|------|------|------|------|
| Agno HITL 在 Workflow 层不完整 | PR review 需降级实现 | 高 | Phase 0.5 验证；降级为独立 Agent 串行调用 |
...
| HITL | Decision Gate + WorkflowPauseRequest（Agno 原生 HITL 作为未来迁移目标） | — |
